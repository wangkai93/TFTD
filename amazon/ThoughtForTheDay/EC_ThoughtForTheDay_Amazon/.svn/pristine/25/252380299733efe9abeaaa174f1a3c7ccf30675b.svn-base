/*
 *
 * ThoughtsAdapter.java created at 2012-2-19 ÏÂÎç01:06:12
 *
 *
...
public class ThoughtsAdapter extends android.widget.BaseAdapter {
	...
	public ThoughtsAdapter(Context context,int viewRourcesID,int[] imageIDs,ArrayList<Event> obj,int[] to,View.OnClickListener listener,boolean isMdpiTablet){
...
//		}
		...
		String tabletCheck = context.getResources().getString(R.string.isTablet);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			holder.relativeLayout = (RelativeLayout)convertView.findViewById(mTo[0]);
		...
		holder.relativeLayout.setBackgroundDrawable(new BitmapDrawable(ReadBitmapUtil.readBitMap(context, imageIDs[temp])));
		
		
		body = "<html><head><style type='text/css'>body{background-color:transparent;} p{background-color:transparent;color:white;font-size:"+bodySize+"px;font-family:Times New Roman,Times,serif;text-align:center;text-shadow:1px 1px 2px #000000;}</style></head><body><p style='font-size:"+titleSize+"px;font-family:Times New Roman,Times,serif;text-align:center;'>"+list.get(temp).getTitle()+"</p>"+list.get(temp).getDescription()+"</body></html>"; 
		setWebViewContent(holder.webView, body);

		if (TFTDApplication.bolFavourite[temp] == 1){
			holder.rightBt.setVisibility(Button.GONE);
		}else{
			holder.rightBt.setBackgroundResource(R.drawable.addfav_button_normal);
		}
		return convertView;
	}
